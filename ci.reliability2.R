ci.reliability2 <- function(data = NULL, S = NULL, N = NULL, aux = NULL, type = "omega", 
                            interval.type = "mlr", B = 10000, conf.level = 0.95) 
# modified from MBESS's "ci.reliability" function. This script uses:
# 1. Revelle and Zinbarg's omega total
# 2. Polychoric correlation for CI bootstrap with bca (alpha, omega total) 
#
...
.ci.reliability.cov <- function(S = NULL, N = NULL, type = NULL, interval.type = NULL, conf.level = 0.95) {
  ...
  if(type == 1) {
    ...
    temp <- suppressWarnings(psych::omega(S, plot=F,nfactors=3,fm="ml",flip=F)$omega.tot)
    relia <- temp
  } else if(type == 4) {
    relia <- suppressWarnings(psych::omega(S, plot=F,nfactors=3,fm="ml",flip=F)$omega_h) #.getH.cov(S, N, varnames)
...
.ci.reliability.data <- function(data = NULL, aux = NULL, type = NULL, interval.type = NULL, B = 1000, conf.level = 0.95) {
  ...
  if(type == 1) {
    ...
    #relia <- .find.alpha(S)
    relia <- suppressWarnings(psych::omega(psych::polychoric(data)$rho, plot=F,nfactors=3,fm="ml",flip=F)$alpha)
    N <- attr(S, "effn")
  } else if(type == 2) {
    ...
    #relia <- temp$relia
    #N <- temp$effn
    relia <- suppressWarnings(psych::omega(psych::polychoric(data)$rho, plot=F,nfactors=3,fm="ml",flip=F)$omega.tot)
    N <- dim(data)[1]
    } else if(type == 4) {
  ...
  if(interval.type == 11) {
    ...
    .bs1 <- function(data, i, varnames) {
      ...
      #.find.alpha(S)
      S <- suppressWarnings(psych::polychoric(data[i,])$rho)
      .find.alpha(S)#suppressWarnings(psych::omega(S, plot=F,nfactors=3,fm="ml",flip=F)$alpha)
    }
    ...
    .bs3 <- function(data, i, varnames, aux) {
      ...
      #relia <- temp$relia
      suppressWarnings(psych::omega(suppressWarnings(psych::polychoric(data[i,])$rho), plot=F,nfactors=3,fm="ml",flip=F)$omega.tot)
    }
